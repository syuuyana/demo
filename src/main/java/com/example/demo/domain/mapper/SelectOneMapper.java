...
public interface SelectOneMapper {
	...
	@Select("SELECT select_id, one_first_times, one_second_times, one_third_times, select_one_date FROM select_one WHERE select_id = #{selectId}")
	SelectOneModel findOne(int selectId); // selectIdはJava側
	
	// そのテーブルの全ての情報を取得する関数
	@Select("SELECT select_id, one_first_times, one_second_times, one_third_times, select_one_date FROM select_one")
